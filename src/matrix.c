...
static void *matmul_p_routine(void *args);
...
void matmul_p(matrix_t matA, matrix_t matB, matrix_t dst)
{
    ...
    arr_thread = (pthread_t *)malloc(sizeof(pthread_t) * len);
    if (arr_thread == NULL)
    {
        perror("malloc");
        exit(EXIT_FAILURE);
    ...
    for (unsigned int tnum = 0; tnum < len; tnum++)
    {
        ...
        ret = pthread_create(arr_thread + tnum, NULL, matmul_p_routine, (void *)args);
        if (ret != 0)
        {
    ...
    for (unsigned int tnum = 0; tnum < len; tnum++)
    {
        ret = pthread_join(arr_thread[tnum], (void **)NULL);
        if (ret != 0)
        {
